    ...
    public partial class MainWindow : Window
    {
        ...
        public MainWindow()
        {
            try
            {
                InitializeComponent();
                init();
                //List<WorkStamp> l = new List<WorkStamp>
                //{
                //    new WorkStamp(DateTime.Now, DateTime.Now, 5),
                //    new WorkStamp(DateTime.Now, DateTime.Now, 5),
                //    new WorkStamp(DateTime.Now, DateTime.Now, 5)
                //};
                //string a = JsonConvert.SerializeObject(l, Formatting.Indented);
                //MessageBox.Show(JsonConvert.SerializeObject(l, Formatting.Indented));
            }
            catch (Exception ex)
            {
        ...
        private void OpenBtn_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (dialog.ShowDialog() == true)
            {
                ...
                file = new StreamWriter(dialog.FileName, true);
        ...
        {
            string output = database.DatabaseToString(month);
        ...
        void addToShowBox(int month)
        {
            ...
            try
            {
                foreach (var item in database.elementsToList())
                {
                    if (month == 0 || item.getStart().Month == month)
                        showTextBox.Items.Add(item);
                }
        ...
        {
            TotalTimeTB.Text = database.calculateMonthHours(month) + " Hours";
        }
